describe('Edit Shipping Line test', function () {

    beforeEach(function () {
        cy.exec("php artisan user:add_permission Supplier-list");
        cy.exec("php artisan user:add_permission Supplier-edit");
        cy.visit('/');
    ...
    it('checks required fields', function () {
        ...
        cy.url().should('contain','/suppliers/1000/edit');
    ...
    it('check nullable fields', function () {
        cy.get('input[name="name"]').clear().type('test');
        cy.get('input[name="phone"]').clear();
        cy.get('input[name="sap_code"]').clear().type('new');
        cy.get('input[name="contact_person"]').clear();
        cy.get('select[name="country_id"] > option[value=""]').invoke('attr', 'selected',true);
        cy.get('.row > :nth-child(2) > .btn-success').click();
        cy.url().should('contain', '/suppliers');

        cy.get('.alert').should('contain','Supplier Updated Successfully')
        cy.server();
        cy.request('get','/api/suppliers/new').then((response)=>{
          console.log(response.body);
          expect(response.body).to.have.property('name', 'test')
          expect(response.body).to.have.property('sap_code', 'new')
    ...
    it('checks edit permissions',function(){
      cy.exec("php artisan user:remove_permission Supplier-edit");
      cy.visit('/suppliers');
      cy.get('.btn-secondary').should('not.exist');
      cy.visit('/suppliers/1000/edit',{ failOnStatusCode: false});
