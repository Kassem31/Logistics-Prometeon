describe('Edit Shipping Line test', function () {
    ...
    beforeEach(function () {
        ...
        cy.exec("php artisan user:add_permission ShippingLine-edit");
        cy.visit('/');
        ...
        cy.visit('/shipping-line/1000/edit');
        cy.get('div.kt-portlet > form').invoke('attr', 'noValidate','true');

    })
    before(function(){
      cy.exec('php artisan test:edit_shipping_line');
    })
    it('checks required fields', function () {
        ...
        cy.url().should('contain','/shipping-line/1000/edit');
    ...
    it('check nullable fields', function () {
        ...
        cy.get('.alert').should('contain','Shipping Line Updated Successfully')
        cy.server();
    ...
    it('checks edit permissions',function(){
      cy.exec("php artisan user:remove_permission ShippingLine-edit");
      cy.visit('/shipping-line');
      cy.get('.btn-secondary').should('not.exist');
      cy.visit('/shipping-line/1000/edit',{ failOnStatusCode: false});
      cy.get('.code').should('contain','403')
      cy.exec("php artisan user:remove_permission ShippingLine-list");
      cy.visit('/shipping-line',{ failOnStatusCode: false});
