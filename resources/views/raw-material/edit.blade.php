...
@section('container')
    <div class='row'>
        <div class='kt-portlet'>
            ...
            <form action='{{ route('raw-materials.update',['raw_material'=>$item]) }}' class='kt-form kt-form--label-right' method='POST' >
                @csrf
                @method('put')
                    ...
                    <div class='kt-section kt-section--first'>
                        <div class='kt-section__body'>
                                ...
                                <div class='col-lg-6'>
                                    <select class='form-control @error('material_group_id') is-invalid @enderror' name='material_group_id'>
                                        ...
                                        @foreach ($groups as $group)
                                            <option value="{{ $group->id }}" {{ $group->id == old('material_group_id',$item->material_group_id) ? 'selected': "" }}>{{ $group->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class='form-group row'>
                                ...
                                <div class='col-lg-6'>
                                    <input type='text' class='form-control @error('hs_code') is-invalid @enderror' placeholder='HS Code' name='hs_code'  value='{{ old('hs_code',$item->hs_code) }}'>
                                    @error('hs_code')
                                        <span class='form-text text-danger'>{{ $message }}</span>
                                    @enderror
                                </div>
