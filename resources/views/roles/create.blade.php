...
@section('container')
    <div class='row'>
        ...
        <div class='kt-portlet__head'>
            ...
            <form action='{{ route('roles.store') }}' class='kt-form kt-form--label-right' method='POST' id="createForm">
                @csrf
                    ...
                    <div class='kt-section kt-section--first'>
                        <div class='kt-section__body'>
                            <div class='form-group row'>
                                 ...
                                 <div class='col-lg-6'>
                                     <input type='text' class='form-control @error('name') is-invalid @enderror' placeholder='Role Name' name='name' value="{{ old('name') }}" autocomplete="off">
                                     @error('name')
                                                ...
                                                <div class="kt-checkbox-list">
                                                    @foreach ($actions as $action)
                                                        <label class="kt-checkbox kt-checkbox--success">
                                                            <input type="checkbox" name="permissions[]" data-name="{{ $action->name }}" value="{{ $action->id }}"> {{ $action->display_name }}
                                                            <span></span>
                                                        </label>
                                                    @endforeach
                                                </div>
